...
module.exports = {
  ...
  messageExists: async function (arg0_channel_id, arg1_message_id) {
    //Convert from parameters
    var channel_id = arg0_channel_id;
    var message_id = arg1_message_id;

    //Declare local instance variables
    var channel_obj = module.exports.returnChannel(channel_id);
    var message_found = true;

    //Check if message can be found
    try {
      await Promise.all([channel_obj.messages.fetch(message_id)]);
    } catch (e) {
      if (e.code == 10008)
        message_found = false;
    } finally {
