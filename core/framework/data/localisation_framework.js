    ...
    try {
      if (all_localisation_keys) {
        ...
        for (var i = 0; i < all_localisation_keys.length; i++) {
          ...
          if (local_replacement.includes("icon:"))
            local_replacement = local_replacement.replace("icon:", "config.icons.");
          ...
          if (options.scopes[local_replacement])
            processed_replacement = options.scopes[local_replacement];
